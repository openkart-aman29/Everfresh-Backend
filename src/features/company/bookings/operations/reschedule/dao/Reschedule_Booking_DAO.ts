...
interface RescheduleData {
    ...
    new_scheduled_time_start: string;
    new_scheduled_time_end?: string | null;
    new_staff_id?: string | null; // 🔥 NEW: Accept staff assignment
}

/**
 * DAO: Reschedule booking
 * Updates scheduled date, time, and staff assignment
 * Sets status to 'rescheduled' if currently 'confirmed'
 */
class RescheduleBookingDAO extends BaseBookingDAO {
    async rescheduleBooking(
        bookingId: string,
        ...
        try {
            ...
            // 🔥 Updated query to include staff_id
            const query = `
                UPDATE bookings
                SET scheduled_date = $1,
                    scheduled_time_start = $2,
            ...
            const values = [
                ...
                rescheduleData.new_scheduled_time_start,
                rescheduleData.new_scheduled_time_end,
                rescheduleData.new_staff_id, // 🔥 NEW: Staff assignment
                bookingId,
                companyId
            ];
