 ...
 * - Cannot reschedule cancelled or completed bookings
 * - Must check staff availability for new time
 * - Auto-assigns staff if current staff unavailable or unassigned
 * - May have rescheduling fee based on notice period
 */
export const rescheduleBookingService = async (
    bookingId: string,
    ...
    try {
        ...
        // ========== STEP 4: Determine Staff Assignment Strategy ==========
        let newStaffId: string | null = null;
        let staffReassignmentReason: string | null = null;
        
        if (currentBooking.staff_id) {
            // ✅ Case 1: Booking has assigned staff - check if they're available at new time
            bookingLogger.info('Checking current staff availability for new time', {
                staff_id: currentBooking.staff_id,
                new_date: rescheduleData.new_scheduled_date,
                new_time: rescheduleData.new_scheduled_time_start
            });
            ...
            if (isAvailable) {
                // Keep same staff
                newStaffId = currentBooking.staff_id;
                staffReassignmentReason = 'STAFF_RETAINED';
                
                bookingLogger.info('Current staff available - retaining assignment', {
                    staff_id: newStaffId
                });
            } else {
                // Current staff not available - try auto-assign
                staffReassignmentReason = 'CURRENT_STAFF_UNAVAILABLE';
                
                bookingLogger.warn('Current staff unavailable - attempting auto-reassignment', {
                ...
                const assignmentResult = await autoAssignStaff(
                    ...
                    rescheduleData.new_scheduled_date,
                    rescheduleData.new_scheduled_time_start,
                    rescheduleData.new_scheduled_time_end || null,
                    bookingId
                );
                
                if (assignmentResult.assigned && assignmentResult.staff_id) {
                    newStaffId = assignmentResult.staff_id;
                    staffReassignmentReason = 'AUTO_REASSIGNED_DUE_TO_UNAVAILABILITY';
                    
                    bookingLogger.info('New staff auto-assigned', {
                        old_staff_id: currentBooking.staff_id,
                    ...
                    // No staff available
                    newStaffId = null;
                    staffReassignmentReason = 'NO_STAFF_AVAILABLE';
                    
                    bookingLogger.warn('No staff available for rescheduled time', {
                        reason: assignmentResult.reason
                    });
                }
            }
        } else {
            // ✅ Case 2: Booking has no assigned staff - try auto-assign
            bookingLogger.info('Booking has no assigned staff - attempting auto-assignment');
            ...
            if (assignmentResult.assigned && assignmentResult.staff_id) {
                newStaffId = assignmentResult.staff_id;
                staffReassignmentReason = 'AUTO_ASSIGNED_ON_RESCHEDULE';
                
                bookingLogger.info('Staff auto-assigned during reschedule', {
                    staff_id: newStaffId
                });
            } else {
                newStaffId = null;
                ...
                bookingLogger.warn('No staff available for assignment', {
                    reason: assignmentResult.reason
                });
            }
        }
        ...
        const rescheduleResult = await rescheduleBookingDAO(
            ...
            companyId,
            {
                ...
                new_scheduled_time_start: rescheduleData.new_scheduled_time_start,
                new_scheduled_time_end: rescheduleData.new_scheduled_time_end,
                new_staff_id: newStaffId // 🔥 Pass new staff assignment
            }
        );
        ...
        if (rescheduleData.notify_customer || rescheduleData.notify_staff) {
            ...
            bookingLogger.info('Reschedule notifications triggered', {
                ...
                old_date: currentBooking.scheduled_date,
                new_date: rescheduleData.new_scheduled_date,
                staff_changed: currentBooking.staff_id !== newStaffId,
                old_staff_id: currentBooking.staff_id,
                new_staff_id: newStaffId
            });
        }
        ...
        bookingLogger.info('Booking rescheduled successfully', {
            booking_id: bookingId,
            booking_number: rescheduleResult.booking.booking_number,
            staff_assignment: staffReassignmentReason,
            old_staff_id: currentBooking.staff_id,
            new_staff_id: newStaffId
        });
