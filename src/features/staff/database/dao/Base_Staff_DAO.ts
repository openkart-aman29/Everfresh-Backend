...
export abstract class BaseStaffDAO {
    ...
    protected getPool(): Pool | null {
        try {
            ...
            staffLogger.error(`Error getting database pool: ${error}`);
            return null;
        }
    }
    ...
    protected logError(operation: string, error: any): void {
        staffLogger.error(`Error in ${operation}: ${error}`, {
            operation,
